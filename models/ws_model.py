...
class WindSpeedPredictionModel(nn.Module):
    def __init__(self, surface_dim, upper_dim, time_series_dim, hidden_dim, num_heads):
        ...
        self.upper_proj = nn.Linear(800*100672, hidden_dim)
    ...
    def forward(self, surface, upper, time_series):
        ...
        upper = upper.view(batch_size * seq_len, *upper_dims)

        surface_features = self.surface_conv(surface).view(batch_size, seq_len, self.surface_conv.out_channels, 11, 11)
        upper_features = self.upper_conv(upper).view(batch_size, seq_len, self.upper_conv.out_channels, 13, 11, 11)

        # Flatten surface and upper features
        surface_features_flat = surface_features.flatten(start_dim=2)  # (batch_size, seq_len, 7744)
        upper_features_flat = upper_features.flatten(start_dim=2)  # (batch_size, seq_len, 93664)

        upper_features_flat_proj = self.upper_proj(
            upper_features_flat)  # Ensure the projection layer maps to the correct dim
